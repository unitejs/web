...
async function findAppFiles(uniteConfig, viewPrefix, cssPrefix) {
    ...
    let files = null;
    try {
        const jsFiles = await globAsync(path.join(uniteConfig.dirs.www.dist, "**/!(app-bundle|vendor-bundle|app-bundle-init|vendor-bundle-init|app-module-config).js"));
        files = jsFiles.map(file => file.replace(/(\.js)$/, ""));
        if (viewPrefix) {
            const viewFiles = await globAsync(path.join(uniteConfig.dirs.www.dist, `**/!(app-bundle|vendor-bundle).${uc.extensionMap(uniteConfig.viewExtensions)}`));
...
async function performAppOptimize(uniteConfig, buildConfiguration, moduleConfig) {
...
gulp.task("build-bundle-app", async () => {
    ...
    if (buildConfiguration.bundle) {
        ...
        try {
            await util.promisify(fs.writeFile)(path.join(uniteConfig.dirs.www.dist, "app-bundle-init.js"), `define(${JSON.stringify(files)}, function () {});`);
        } catch (err) {
